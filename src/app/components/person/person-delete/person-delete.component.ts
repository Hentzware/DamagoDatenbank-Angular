...
export class PersonDeleteComponent {
  ...
  constructor(private dialogRef: MatDialogRef<PersonDeleteComponent>, @Inject(MAT_DIALOG_DATA) public data: any, private personService: PersonService, private addressService: AddressService) {
    this.last_name = data.person.last_name;
    this.first_name = data.person.first_name;
  }

  public save(): void {
    this.personService.delete(this.data.person.id).pipe(concatMap(() => {
